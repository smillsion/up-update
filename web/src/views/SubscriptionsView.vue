...
async function load(){loading.value=true;error.value='';try{const [subscriptions,userSettings]=await Promise.all([request<Subscription[]>('/subscriptions'),request<Settings>('/settings')]);items.value=subscriptions;settings.value=userSettings}catch(e){error.value=e instanceof Error?e.message:'加载失败'}finally{loading.value=false}}
