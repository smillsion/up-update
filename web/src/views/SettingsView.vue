...
<template><section class="page narrow"><header class="page-header"><div><p class="eyebrow">个人配置</p><h1>设置</h1></div></header>
  ...
  <section class="settings-section"><header class="section-heading"><span class="section-icon green"><Smartphone :size="20"/></span><div><h2>Bark 推送</h2><p>{{settings?.bark.configured?'已配置 Device Key':'尚未配置'}}</p></div></header><form @submit.prevent="saveBark"><label>Bark Server<input v-model.trim="server" type="url" required /></label><label>Device Key<input v-model.trim="deviceKey" type="password" autocomplete="off" :placeholder="settings?.bark.configured?'已安全保存；输入新 Key 可替换':'Bark 中显示的推送 Key'" :required="!settings?.bark.configured" /></label><div class="form-grid"><label>通知级别<select v-model="level"><option value="active">主动通知</option><option value="timeSensitive">时效性通知</option><option value="passive">静默通知</option><option value="critical">重要警报</option></select></label><label>提示音<select v-model="soundChoice"><option value="">默认提示音</option><option v-for="item in commonSounds" :key="item" :value="item">{{item}}</option><option value="custom">自定义名称</option></select></label></div><label v-if="soundChoice==='custom'">自定义提示音名称<input v-model.trim="customSound" placeholder="输入 Bark 中的音效名称" required /></label>
    <div class="quiet-settings"><div class="setting-toggle"><div><strong>午休静默</strong><small>开启后，时段内自动通知以静默级别发送</small></div><label class="switch" title="切换午休静默"><input v-model="quietEnabled" type="checkbox" aria-label="午休静默"/><span/></label></div><div class="form-grid"><label>开始时间<input v-model="quietStart" type="time" :disabled="!quietEnabled" required /></label><label>结束时间<input v-model="quietEnd" type="time" :disabled="!quietEnabled" required /></label></div></div>
