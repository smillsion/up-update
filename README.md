...
项目适合个人、家人和朋友共同使用的小规模自托管场景。每位用户独立登录系统，配置自己的 UP 主订阅和 Bark 推送；B 站登录仅在导入关注列表时需要，不共享 Cookie 或推送密钥。
...
| 可选 B 站登录 | UID 和空间链接可直接订阅；扫码登录后可从当前账号关注列表批量导入 |
| 灵活订阅 | 公共投稿始终匿名轮询，并显示最新投稿时间与距今时长 |
...
| 1. 登录 | 2. 订阅 | 3. 监控 | 4. 推送 |
| --- | --- | --- | --- |
| 用户登录系统并配置 Bark | 直接添加 UID、空间链接，或可选登录 B 站后从关注列表导入 | 后台匿名检查 UP 主最新投稿 | 发现新视频后通过 Bark 推送到 iPhone |
