...
每位用户可在“设置”页面独立配置 B 站登录。UID 和空间链接订阅不要求登录；配置登录后可以导入关注列表，并提升投稿轮询稳定性。
...
扫码登录保存 Cookie 和刷新令牌；手工保存 Cookie 时会删除该用户已有的刷新令牌。后台定期验证登录状态，并优先选择订阅该 UP 主的有效用户 Cookie 查询公共投稿；不会使用未订阅该 UP 主的用户凭证。认证失效时会标记凭证并尝试下一位订阅者，没有可用凭证时匿名兜底；`-352`、HTTP `412` 等风控错误不会触发连续切换账号，仍按轮询失败退避处理。
